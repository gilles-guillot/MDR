---
title: "The MDR package for dose-response analysis of toxicological and pharmacological mixtures with the model deviation ratio method."
...
## Single ray design
Under this setting, it is assumed that data consist of single compound experiment data for each compound, and mixture experiment data where the dose of both compounds varies while the ratio of their doses is held constant.  On such a ray, even though both doses vary, there is only one degree of freedom. This can be re-parameterized for example as $(x_i,y_i)= d_i(w,1-w)$ where $d_i=x_i+y_i$ and $w=x_i/(x_i+y_i)$ 

or  as $(x_i,y_i) = y_i(a,1)$ where  $a=x_i/y_i$.

Using one of those parametrizations make it possible to use methods designed for single compound experiment data. 

###  Parameterization  $d_i(w,1-w)$ 

The dose additivity equation $x_e / X_e +  y_e/Y_e = 1$ can be written 
