...
A subset of the numerical data contained as table 1 and 2 in @dalla2014sensitivity is provided as R objects `dallabona.sc` and `dallabonna.mix`.  These data are used below to show how the MDr can be computed together with an estimate of the uncertainty. 
...
# par(mfrow=c(1,3))
for(i in sort((1:10)[dallabona.sc[,1] %in% c('SGD','EFX','CPX')],decreasing=TRUE))
{
  r =  (dallabona.sc[i,'max'] / dallabona.sc[i,'min'])^(1/4)
  r
  doses = dallabona.sc[1,'min']*c(1,r,r^2,r^3,r^4)
  dr_func = function(doses,EC50,Hill.Slope){ 1/(1+10^((log10(EC50) - log10(doses))*Hill.Slope)) }

  dd = seq(1,500,l=1000)
  plot(dd,dr_func(doses=dd,EC50=dallabona.sc[i,'EC50'],Hill.Slope = dallabona.sc[i,'Hill.Slope']),
       type='l',log='x', 
       xlab=expression(paste(g.,L ^{-1})),
       ylab = expression(paste(Immobilizartion,"%",sep=" ")),
       main=dallabona.sc[i,1],lwd=2)
}
...
for(i in 1:4)
{
  ...
  plot(dsc.a,dsc.b,type="n", xlab=name.a, ylab=name.b,cex=1.5,
       xlim=c(0,dallabona.sc[i.a,'max']),
       ylim=c(0,dallabona.sc[i.b,'max']))
  ...
  segments(dallabona.sc[dallabona.sc$Compound==name.a,'EC50'],0,
           0,dallabona.sc[dallabona.sc$Compound==name.b,'EC50'],col='red',lty=1)
  ## loop on various mixtures proportions for this particular combination of compounds
  for(j in (1:12)[dallabona.mix[,1] == name.a &  dallabona.mix[,2] == name.b] )
  {
    w.max = dallabona.mix[j,'max.b'] / dallabona.mix[j,'max.a']
    w.min = dallabona.mix[j,'min.b'] / dallabona.mix[j,'min.a']
    ...
    points(dallabona.mix[j,'EC50.a'], dallabona.mix[j,'EC50.b'],pch=1,cex=1.5,col='red')
    segments(dallabona.mix[j,'EC50.a.low'],dallabona.mix[j,'EC50.b.low'],
             dallabona.mix[j,'EC50.a.up'],dallabona.mix[j,'EC50.b.up'],col='red',lwd=3)
  }
}
```
...
for(i in 1:4) ## loop on the four compound combinations
{
  ...
  for(j in (1:12)[dallabona.mix[,1] == name.a &  dallabona.mix[,2] == name.b] ) # loop on compound combination x mixture proportions
  {
    w.max = dallabona.mix[j,'max.b'] / dallabona.mix[j,'max.a']
    r.a = (dallabona.mix[j,'max.a'] / dallabona.mix[j,'min.a'])^(1/4)
    dm.a = dallabona.mix[j,'min.a'] * c(1,r.a,r.a^2,r.a^3,r.a^4)
